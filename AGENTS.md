...
- 平均間隔（日）
- カウントダウン（今日から予測日まで）
- 予測番号（`predicted_version`、後述）

### 予測番号 (predicted_version)

- 最新バージョン文字列から次の版番号を機械的に推測する
- 断定を避けるため末尾に必ず `?` を付ける
- アルゴリズム:
  1. 文字列中で最初に現れる `N.M[.O...]`（ドット区切り数列）の最下位コンポーネントを +1
  2. 見つからなければ最初の整数 `N` を +1
  3. どちらも見つからなければ `null`（UI では省略）
- 残りの文字列は保持する
- 例: `4.7` → `4.8?`、`GPT-5.4` → `GPT-5.5?`、`v3` → `v4?`、`Qwen3.6-35B-A3B` → `Qwen3.7-35B-A3B?`、`Nano Banana Pro` → `null`
- 主要バージョンのジャンプ（`3.9` → `4.0`）や新ブランド切替は予測しない。単純インクリメントに限定し、占い化を避ける
